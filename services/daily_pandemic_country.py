...
def add_daily_data(id_country, id_pandemic, date, daily_new_deaths, daily_new_cases):
    conn = connect_to_db()
    with conn.cursor() as cursor:
        cursor.execute("""
...
def update_daily_data(id_country, id_pandemic, date, daily_new_deaths, daily_new_cases):
    conn = connect_to_db()
    with conn.cursor() as cursor:
        cursor.execute("""
            UPDATE daily_pandemic_country
            SET "daily_new_deaths" = %s, "daily_new_cases" = %s
            WHERE "id_country" = %s AND "id_pandemic" = %s AND "date" = %s
        """, (daily_new_deaths, daily_new_cases, id_country, id_pandemic, date))
    conn.commit()
    conn.close()
